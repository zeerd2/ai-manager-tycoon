import type { Achievement } from '../domain/achievement';

export const achievements: Achievement[] = [
  {
    id: 'first-blood',
    name: '一血',
    emoji: '🎯',
    description: '完成第一个项目',
    conditionType: 'first_project_completed',
    category: 'project',
    rarity: 'common',
  },
  {
    id: 'bug-factory',
    name: 'Bug 批发商',
    emoji: '🏭',
    description: '单轮产生 15+ bugs',
    conditionType: 'single_sprint_15_bugs',
    category: 'incident',
    rarity: 'common',
  },
  {
    id: 'under-budget',
    name: '勤俭持家',
    emoji: '💰',
    description: '完成项目时剩余资金超过 80%',
    conditionType: 'complete_project_80_percent_funds',
    category: 'project',
    rarity: 'rare',
  },
  {
    id: 'team-wipe',
    name: '全员自闭',
    emoji: '💀',
    description: '所有员工同时 morale 为 0',
    conditionType: 'all_agents_zero_morale',
    category: 'employee',
    rarity: 'legendary',
  },
  {
    id: '10x-company',
    name: '10x 神级公司',
    emoji: '🚀',
    description: '一局游戏内完成 3 个以上项目',
    conditionType: 'three_projects_one_game',
    category: 'project',
    rarity: 'epic',
  },
  {
    id: 'speed-run',
    name: '速通狂魔',
    emoji: '⚡',
    description: '5 轮内完成一个项目',
    conditionType: 'project_in_5_sprints',
    category: 'project',
    rarity: 'rare',
  },
  {
    id: 'iron-man',
    name: '钢铁之躯',
    emoji: '🦾',
    description: '让一个员工连续参与 6 轮不休息',
    conditionType: 'agent_6_consecutive',
    category: 'employee',
    rarity: 'rare',
  },
  {
    id: 'penny-pincher',
    name: '葛朗台',
    emoji: '🪙',
    description: '只用最便宜的员工完成一个项目',
    conditionType: 'cheapest_agent_only',
    category: 'economic',
    rarity: 'epic',
  },
  // 8 个新增成就
  {
    id: 'max-skill',
    name: '满级大佬',
    emoji: '👑',
    description: '培养一名员工技能全满 (所有技能达到 100)',
    conditionType: 'agent_max_skills',
    category: 'employee',
    rarity: 'legendary',
  },
  {
    id: 'big-team',
    name: '人才济济',
    emoji: '👥',
    description: '同时解锁 6 名以上员工',
    conditionType: 'six_agents_unlocked',
    category: 'employee',
    rarity: 'rare',
  },
  {
    id: 'talent-scout',
    name: '伯乐',
    emoji: '🐴',
    description: '培养出一名总技能点数达到 450 的明星员工',
    conditionType: 'five_star_agent',
    category: 'employee',
    rarity: 'epic',
  },
  {
    id: 'legendary-project',
    name: '地狱难度通关',
    emoji: '👹',
    description: '完成一个难度大于等于 80 的传说项目',
    conditionType: 'complete_legendary_project',
    category: 'project',
    rarity: 'legendary',
  },
  {
    id: 'financial-freedom',
    name: '财务自由',
    emoji: '🏛️',
    description: '公司剩余资金达到 8000 以上',
    conditionType: 'funds_reach_8000',
    category: 'economic',
    rarity: 'common',
  },
  {
    id: 'big-spender',
    name: '散财童子',
    emoji: '💸',
    description: '单局累计花费超过 10000 资金',
    conditionType: 'spend_10000_funds',
    category: 'economic',
    rarity: 'common',
  },
  {
    id: 'survivor',
    name: '大难不死',
    emoji: '🩹',
    description: '成功在一轮产生 15 个以上 bugs 之后把项目完成',
    conditionType: 'recover_from_bugs',
    category: 'incident',
    rarity: 'rare',
  },
  {
    id: 'murphy-law',
    name: 'Murphy 定律',
    emoji: '🤡',
    description: '单局游戏内经历并产生过 50 个以上的 bug',
    conditionType: 'fifty_bugs_total',
    category: 'incident',
    rarity: 'common',
  },
  // v9 low-risk achievements (G-2A)
  {
    id: 'long-run-survivor',
    name: '长期经营者',
    emoji: '🕰️',
    description: '坚持经营超过 20 个 Sprint',
    conditionType: 'long_run_survivor',
    category: 'project',
    rarity: 'rare',
  },
  {
    id: 'efficient-project',
    name: '高效执行者',
    emoji: '⚡',
    description: '完成过高进度且低成本的项目',
    conditionType: 'efficient_project',
    category: 'project',
    rarity: 'epic',
  },
  {
    id: 'fast-unlock',
    name: '快速扩张',
    emoji: '🚀',
    description: '在 10 轮内解锁 5 名以上员工',
    conditionType: 'fast_unlock',
    category: 'employee',
    rarity: 'rare',
  },
  {
    id: 'bug-survivor-streak',
    name: 'Bug 幸存者',
    emoji: '🛡️',
    description: '在产生大量 Bug 的 Sprint 后仍推进项目',
    conditionType: 'bug_survivor_streak',
    category: 'incident',
    rarity: 'rare',
  },
  {
    id: 'stable-team',
    name: '团队稳定',
    emoji: '🧘',
    description: '长期维持团队士气稳定',
    conditionType: 'stable_team',
    category: 'employee',
    rarity: 'epic',
  },
];